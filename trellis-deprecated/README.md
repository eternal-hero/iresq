...
`./bin/deploy.sh production production-iresq.com` <— deploy to production

`./bin/deploy.sh staging staging-iresq.com` <— deploy to staging

`vagrant trellis-sequel open` —> View Database

`EDITOR=nano ansible-vault edit group_vars/production/vault.yml` —> Edit Production Secrets
