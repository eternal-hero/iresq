...
@if ( $has_orders )
	...
	<table class="woocommerce-orders-table woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_orders account-orders-table">
		...
		<tbody>
			@foreach ( $customer_orders->orders as $customer_order )
				@php
				$po_number 		= get_post_meta( $customer_order->get_id(), 'Client PO#', true );
	...
	@if ( 1 < $customer_orders->max_num_pages )
		<div class="woocommerce-pagination woocommerce-pagination--without-numbers woocommerce-Pagination">
			@if ( 1 !== $current_page )
				<a class="woocommerce-button woocommerce-button--previous woocommerce-Button woocommerce-Button--previous button" href="{{ esc_url( wc_get_endpoint_url( 'orders', $current_page - 1 ) ) }}">@php esc_html_e( 'Previous', 'woocommerce' ); @endphp</a>
			@endif

			@if ( intval( $customer_orders->max_num_pages ) !== $current_page )
				<a class="woocommerce-button woocommerce-button--next woocommerce-Button woocommerce-Button--next button" href="{{ esc_url( wc_get_endpoint_url( 'orders', $current_page + 1 ) ) }}">@php esc_html_e( 'Next', 'woocommerce' ); @endphp</a>
			@endif
		</div>
	@endif
...
@else
	<div class="woocommerce-message woocommerce-message--info woocommerce-Message woocommerce-Message--info woocommerce-info">
		<a class="woocommerce-Button button" href="{{ esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ) }}">@php esc_html_e( 'Browse products', 'woocommerce' ); @endphp</a>
